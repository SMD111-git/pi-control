...
print(
    f"[GPIO] LEFT: "
    f"PWMA={PWMA} AIN1={AIN1} AIN2={AIN2} STBY={STBY_LEFT}"
)

print(
    f"[GPIO] RIGHT: "
    f"PWMB={PWMB} BIN1={BIN1} BIN2={BIN2} STBY={STBY_RIGHT}"
)
...
def start_new_recording():
    ...
    print(
        f"[RECORDING] New recording: "
        f"{current_recording_id}"
    )
...
def send_motor_command(
    ...
    if command == "STOP":
        ...
        left_pwm, right_pwm = apply_motor_gpio(
            command,
            speed
        )
...
def detect_black_lane(frame):
    ...
    left_distance = max(
        0,
        CAR_CENTER_X - lane_left_x
    )

    right_distance = max(
        0,
        lane_right_x - CAR_CENTER_X
    )
...
def generate_frames():
    ...
    while running:
        ...
        try:
            ...
            overlay, lane_data = detect_black_lane(
                frame
            )
...
labels.csv
    Image labels and lane information
...
Each image is linked to its command through
the image_filename column in labels.csv.
...
<p>
All images and logs are stored in one location.
There are no session folders.
</p>

<button
class="new-session"
id="new_recording_button"
>
START NEW RECORDING
</button>
...
<a
href="/download_dataset"
>
DOWNLOAD COMPLETE CAR4 DATASET ZIP
</a>

<a
href="/download_labels"
>
DOWNLOAD LABELS CSV
</a>

<a
href="/download_motor_log"
>
...
document
    ...
    .addEventListener(
        ...
        () => {
            ...
            if (
                !confirm(
                    "Start a new recording? Existing images and CSV data will remain."
                )
            ) {
...
document
    ...
    .addEventListener(
        ...
        () => {
            ...
            if (
                !confirm(
                    "DELETE ALL CAR4 IMAGES, CSV LOGS AND ZIP DATA? This cannot be undone."
                )
            ) {
